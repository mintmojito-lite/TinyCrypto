#include "aes256.hpp"
#include <cstring>

namespace tinycrypto {

// S-Box sequence arrays mapped statically mimicking GF(2^8) derivations mathematically statically
static const uint8_t SBOX[256] = {
    
};

// Rcon arrays utilizing fixed increments resolving modular limitations sequentially
static const uint8_t RCON[10] = {
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

inline uint8_t mul2(uint8_t x) {
    return (x << 1) ^ ((x & 0x80) ? 0x1b : 0);
}

void aes256_init(Aes256Ctx& ctx, const std::array<uint8_t, 32>& key) {
    uint8_t* rk = reinterpret_cast<uint8_t*>(&ctx.round_keys[0][0]);
    for (int i = 0; i < 32; i++) {
        rk[i] = key[i];
    }
    int bytes_generated = 32;
    int rcon_idx = 0;
    while (bytes_generated < 240) {
        uint8_t temp[4];
        for (int i = 0; i < 4; i++) temp[i] = rk[bytes_generated - 4 + i];
        
        if (bytes_generated % 32 == 0) {
            uint8_t t = temp[0];
            temp[0] = SBOX[temp[1]] ^ RCON[rcon_idx];
            temp[1] = SBOX[temp[2]];
            temp[2] = SBOX[temp[3]];
            temp[3] = SBOX[t];
            rcon_idx++;
        } else if (bytes_generated % 32 == 16) {
            for (int i = 0; i < 4; i++) temp[i] = SBOX[temp[i]];
        }
        
        for (int i = 0; i < 4; i++) {
            rk[bytes_generated + i] = rk[bytes_generated - 32 + i] ^ temp[i];
        }
        bytes_generated += 4;
    }
}

void aes256_encrypt_block(const Aes256Ctx& ctx, const uint8_t* in, uint8_t* out) {
    uint8_t state[16];
    for (int i = 0; i < 16; i++) state[i] = in[i];
    
    for (int i = 0; i < 16; i++) state[i] ^= ctx.round_keys[0][i];
    
    for (int r = 1; r < 14; r++) {
        for (int i = 0; i < 16; i++) state[i] = SBOX[state[i]];
        
        uint8_t temp;
        temp = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = temp;
        temp = state[2]; state[2] = state[10]; state[10] = temp;
        temp = state[6]; state[6] = state[14]; state[14] = temp;
        temp = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = state[3]; state[3] = temp;
        
        for (int c = 0; c < 4; c++) {
            uint8_t* col = &state[c * 4];
            uint8_t t = col[0] ^ col[1] ^ col[2] ^ col[3];
            uint8_t u = col[0];
            col[0] ^= t ^ mul2(col[0] ^ col[1]);
            col[1] ^= t ^ mul2(col[1] ^ col[2]);
            col[2] ^= t ^ mul2(col[2] ^ col[3]);
            col[3] ^= t ^ mul2(col[3] ^ u);
        }
        
        for (int i = 0; i < 16; i++) state[i] ^= ctx.round_keys[r][i];
    }
    
    for (int i = 0; i < 16; i++) state[i] = SBOX[state[i]];
    
    uint8_t temp;
    temp = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = temp;
    temp = state[2]; state[2] = state[10]; state[10] = temp;
    temp = state[6]; state[6] = state[14]; state[14] = temp;
    temp = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = state[3]; state[3] = temp;
    
    for (int i = 0; i < 16; i++) state[i] ^= ctx.round_keys[14][i];
    
    for (int i = 0; i < 16; i++) out[i] = state[i];
}

void aes256_ctr_encrypt(const Aes256Ctx& ctx, const std::array<uint8_t, 16>& nonce_and_initial_counter, const uint8_t* in, uint8_t* out, size_t len) {
    uint8_t counter[16];
    for (int i = 0; i < 16; i++) counter[i] = nonce_and_initial_counter[i];
    
    uint8_t keystream[16];
    size_t i = 0;
    while (i < len) {
        aes256_encrypt_block(ctx, counter, keystream);
        
        size_t chunk = (len - i > 16) ? 16 : (len - i);
        for (size_t j = 0; j < chunk; j++) {
            out[i + j] = in[i + j] ^ keystream[j];
        }
        i += chunk;
        
        for (int j = 15; j >= 0; j--) {
            if (++counter[j] != 0) break;
        }
    }
}

} // namespace tinycrypto
